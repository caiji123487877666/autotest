...
def read_files_once(folder_path):
    ...
    all_files = list(file_generator(folder_path))  # 生成文件列表
    file_time = time.time() - start_time  # 计算获取文件名称所花费的时间
    if not all_files:
        logging.warning("没找到文件路径")
        return 0, 0, 0.0  # 返回默认时间 0.0
...
def main():
    ...
    logging.info(f"所有读取完成。成功: {success}，失败: {failed}")
    logging.info(f"获取文件名称总时间: {file_time:.2f} 秒")
